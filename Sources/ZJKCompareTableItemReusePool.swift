//
//  ZJKCompareTableItemReusePool.swift
//  ZJKCompareTable
//
...
class ZJKCompareTableItemReusePool: NSObject {

    /// 取出可重用的视图
    /// - Returns: 可重用的视图
    public func dequeueReusableView(with identifier: String) -> ZJKCompareTableItemCell? {
        guard let view = waitUsedPool.first as? ZJKCompareTableItemCell else {
            return nil
        }
    ...
    public func addUsing(_ view: ZJKCompareTableItemCell?) {
    ...
    /// 移除正在使用的视图
    /// - Parameter view: 正在使用的视图
    public func removeUsing(_ view: ZJKCompareTableItemCell?) {
        guard let newView = view else {
            return
        }
